...
namespace MediaPipe.FaceLandmark
{
  ...
  public sealed class FaceLandmarkDetector : System.IDisposable
  {
    ...
    public FaceLandmarkDetector(ResourceSetFace resources)
    {
    ...
    void RunModel(Texture source)
    {
      ...
      // Debug.Log("_postBuffer:" + _postBuffer);
    ...
    public Vector4[] _postReadCache = new Vector4[VertexCount];
    bool _postRead;

    public Vector4[] UpdatePostReadCache()
    {
      _postBuffer.GetData(_postReadCache, 0, 0, VertexCount);
      // Debug.Log("_postReadCache:" + _postReadCache);
      _postRead = true;
